...
class SnapshotExecutionGateway:
    ...
    def _load_invocation(self, snapshot_hash, request):
        row = self.store.conn.execute(
            "SELECT snapshot_object_id FROM execution_snapshots WHERE snapshot_hash = ?",
            (snapshot_hash,),
        ).fetchone()
        if row is None:
        ...
        config_value = (
            json.loads(self.store.runtime.read_text(config.config_object_id))
        ...
        if snapshot.resolved_credential_version_id:
            credential = self.credential_store.read(
                snapshot.resolved_credential_version_id
            )
        ...
        return snapshot, artifact, payload, frozen_limits

    def invoke(self, snapshot_hash, operation, request, *, limits=None):
        try:
            if limits is not None:
                raise SupplierRuntimeUnavailable("SUPPLIER_RUNTIME_UNAVAILABLE")
            snapshot, artifact, payload, frozen_limits = self._load_invocation(
                snapshot_hash, request
            )
            worker_options = {"mode": "execution", "limits": frozen_limits}
            if snapshot.worker_protocol_version != "1":
                worker_options["worker_protocol_version"] = (
                    snapshot.worker_protocol_version
                )
            value = self.worker.invoke(
                artifact, operation, payload, **worker_options
            ).value
    ...
    def invoke_stream(self, snapshot_hash, operation, request, *, limits=None):
        try:
            if limits is not None:
                raise SupplierRuntimeUnavailable("SUPPLIER_RUNTIME_UNAVAILABLE")
            snapshot, artifact, payload, frozen_limits = self._load_invocation(
            ...
            if (
                snapshot.worker_protocol_version != "2"
                or snapshot.helper_api_version != "ai-drama-helper-v3"
            ):
                raise SupplierRuntimeUnavailable("SUPPLIER_RUNTIME_UNAVAILABLE")
